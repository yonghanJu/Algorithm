class Solution {
    ...
    fun solution(n: Int, weak: IntArray, dist: IntArray): Int {
        dfs(0, 0, weak.toSet(), dist.sortedBy { -it }, n)

        return if (answer == Int.MAX_VALUE) -1 else answer
    }

    fun dfs(distIndex: Int, distCount: Int, remainWeaks: Set<Int>, dist: List<Int>, n: Int) {
        if ((answerMap[remainWeaks] ?: Int.MAX_VALUE) <= distCount) return
        answerMap[remainWeaks] = distCount
        ...
        remainWeaks.forEach { startIndex ->
            ...
            dfs(distIndex + 1, distCount + 1, remain, dist, n)
        }
        dfs(distIndex + 1, distCount, remainWeaks, dist, n)
    }
}
